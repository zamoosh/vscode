...
from ticket.models import Ticket, Message, Latency
...
class TicketSingleSerializer(BaseTicketSerializer):
    ...
    def get_gps(cls, obj: Ticket):
        try:
            ...
            cls.CAR_STATE.aggregate_query.append(
                {
                    "$match": {
                        "carID": (
                            car.device.imei
                            if car.old_id is None
                            else bson.ObjectId(car.old_id)
                        ),
                        "coordinates": {"$ne": None},
                    }
                }
            ...
            cls.CAR_STATE.aggregate_query.append(
                {"$unwind": {"preserveNullAndEmptyArrays": True, "path": "$lpt"}}
            )
            cls.CAR_STATE.aggregate_query.append(
                {"$addFields": {"last_post_time": "$lpt.postTime"}}
            )
            cls.CAR_STATE.aggregate_query.append({"$limit": 1})
            data = list(cls.CAR_STATE.aggregate(cls.CAR_STATE.aggregate_query))
...
class ReadLatencySerializer(serializers.ModelSerializer):
    class Meta:
        model = Latency
        fields = ["id", "ticket", "user", "duration", "created_at"]

    def get_ticket(self, obj: Latency):
        return "ticket"
