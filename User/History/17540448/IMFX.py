...
class ReadCatalogueSerializer(serializers.ModelSerializer):
    class Meta:
        model = Catalogue
        fields = [
            'id', 'book', 'parent', 'name', 'slug', 'sub_name', 'created_at', 'updated_at', 'start_page', 'order',
            'content_count'
        ]
...
class WriteCatalogueSerializer(serializers.Serializer):
    ...
    start_page = serializers.IntegerField(allow_null=True, required=False)
    ...
    def validate(self, attrs: dict):
        ...
        elif (self.b.view_type == Book.DEDICATED) and (attrs.get("start_page") is not None):
    ...
    def validate_parent(self, value):
        if value is not None:
            ...
            self.parent_obj = Catalogue.objects.filter(deleted_at=None, id=value).first()
            if self.parent_obj is None:
                raise serializers.ValidationError("ترجمه کاتالوگ یافت نشد")

            if self.instance is not None:
                if self.instance.get_family().filter(deleted_at=None, id=value).exists():
                    raise serializers.ValidationError("پدر، نمیتواند از فرزندان یک فصل انتخاب شود")
    ...
    def validate_start_page(self, value):
        if value is not None:
            try:
                ...
                raise serializers.ValidationError("شماره صفحه‌ی آغازین فصل، باید عددی نامنفی باشد")
