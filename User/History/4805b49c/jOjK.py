...
from typing import Iterable, Any, TypeGuard
...
def only_series(item) -> TypeGuard[Iterable[Series]]:
    """
    If item is of type "Seirse", return it, otherwise skip it.
    """
...
class Parser:
    ...
    def parser(
        ...
        for sheet_name in xls.sheet_names:
            ...
            catalogue_name: str = (
                f"{lead_catalogue_name} "
                f"{Parser.to_other_numerics(number[0], numeric_sys)}: "
            )
            ...
            if lead_catalogue_name == "حکمت":
                original_catalogue_name: str = (
                    f"الحكمة " f"{Parser.to_other_numerics(number[0], Parser.ar)}: "
                )
            elif lead_catalogue_name == "خطبه‌ی":
                original_catalogue_name: str = (
                    f"خطبة " f"{Parser.to_other_numerics(number[0], Parser.ar)}: "
                )
            elif lead_catalogue_name == "نامه‌ی":
                original_catalogue_name: str = (
                    f"الحرف " f"{Parser.to_other_numerics(number[0], Parser.ar)}: "
                )
            ...
            for row in df.iterrows():
                ...
                ser_list: Iterable[Series] = filter(only_series, row)

                # === SET PAGE NUMBER === #
                page_number = None
                try:
                    # page_number = int(row[-1])
                    page_number = int(ser_list[len(ser_list) - 1])
                ...
                for ser in ser_list:
                    ...
                    for col in range(len(ser)):
                        ...
                        # === ADD "Catalogue" NAME === #
                        # if col == 1 and ser[0] == 0:
                        if col == 0:
                            if main_catalogue:
                                original_catalogue_name += str(ser[col]).strip()

                                original_catalogue = Catalogue.objects.get(
                                    name=original_catalogue_name
                                )

                            catalogue_name += str(ser[col].strip())
                        ...
                        if col == starter_column:  # 3
                            ...
                            if main_content:
                                original_content_body += f"<h2>{ser[3].strip()}</h2>\n\n"

                            content_body += f"<h2>{ser[col].strip()}</h2>\n\n"

                        if col == starter_column + 2:  # 5
                            if main_content:
                                original_content_body += ser[5].strip()
    ...
    def parse_nameh_ar():
        ...
        with pd.ExcelFile("/home/zamoosh/Downloads/nameh.xlsx") as xls:
            Parser.parser(
                xls,
                book,
                parent,
